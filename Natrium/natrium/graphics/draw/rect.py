...
def render_round_rect(rect:pygame.Rect, color:IsColor, radius:int = 0, antialias:bool = True, gaussian:int = 0):
    gaussian_offset = gaussian*2+1
    main_surf = pygame.Surface([x+2+gaussian_offset for x in rect.size], pygame.SRCALPHA, 32)
    # main_surf.fill('red')
    ...
    deflated_rt.topleft = (max_radius+gaussian_offset//2+1, max_radius+gaussian_offset//2+1)
    ...
    for i, (x, y) in enumerate([[rt_coords[0], rt_coords[1]], [rt_coords[1], rt_coords[2]],
        ...
        if i % 2 == 0:
            ...
            if i == 1:
                x2 = [x[0]+length_offsetx, x[1]+height_offsetx]
                y2 = [y[0]-length_offsety, y[1]-height_offsety] # this code is worthy of my anus

            else:
                x2 = [x[0]+length_offsetx, x[1]+height_offsetx] # cant even work properly
                y2 = [y[0]-length_offsety, y[1]+height_offsety]
...
def render_round_rect_gradient(rect:pygame.Rect, colors:list[IsColor], radius:int = 0, antialias:bool = True,
                            gaussian:int = 0, orient="horizontal", gradient_blend:int = 10):

    main_surf = pygame.Surface([x+2 for x in rect.size], pygame.SRCALPHA, 32)
    grad_surf = color.gradient([x+4+gaussian*4 for x in rect.size], colors, orient, gradient_blend)
...
def render_dynamic_rect(rect:pygame.Rect, colors:clcs.Sequence[IsColor], radius:int, antialias:bool = True,
    ...
    elif isinstance(colors[0], (list, tuple, pygame.Color, str)) and len(colors):
        return render_round_rect_gradient(rect, colors, radius, antialias, gaussian, orient, gradient_blend)

    else:
        raise ValueError("Colors argument must be a Sequence[IsColor] with a length of 2 or more or a single color.")
...
def instant_round_rect_gradient(surface:pygame.Surface, rect:pygame.Rect, colors: list[IsColor], radius:int = 0,
                             antialias:bool = True, gaussian:int = 0):
    surf = render_round_rect_gradient(rect, colors, radius, antialias, gaussian)
    surface.blit(surf, rect.topleft)
