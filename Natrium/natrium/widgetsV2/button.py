from natrium import widgetsV2, common, display
...
class Button(widgetsV2.Label):
    ...
    def __init__(self, container, text="", style=None, font=('gadugi', 12), anchor='topleft', size=None, padding=(10, 10),
                 image_path=None, image_anchor='midleft', image_margin=(5, 5), click_once=True, command=lambda: None):
    ...
    def trigger(self, events, mpos, mprd):
        ...
        if self.rect.collidepoint(*mpos):
            ...
            Button.triggered_widgets.append(self)
        ...
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN:
                if self.rect.collidepoint(*event.pos) and event.button == 1: self.command()

        if self._event_mode == 2 and not self.click_once:
            self.command()
    ...
    def place_hierarchy(self):
        ...
        self._hierarchy_rendering[2].blit(self._image, image_position)
